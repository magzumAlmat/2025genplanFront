...
//           <GisMap onMarkerPositionChange={handleMarkerPositionChange}/>
//           </Col>
...
export default function AddBanner() {
  ...
  // Состояние для управления этапами
  const [step, setStep] = useState(1); // 1: выбор точки, 2: заполнение формы
  ...
  const tariffsByViewOfAd2 = [{ id: 1, tariffName: 'Все размеры' }];
  const tariffsByViewOfAd3 = [
    { id: 1, tariffName: 'до 30 кв.м' },
    { id: 2, tariffName: 'свыше 30 кв.м' },
  ];
  ...
  const tariffsByViewOfAd6 = [{ id: 1, tariffName: 'Все размеры' }];
  const tariffsByViewOfAd7 = [
    { id: 1, tariffName: 'до 20 кв.м' },
    { id: 2, tariffName: 'свыше 20 кв.м' },
  ];
  const tariffsByViewOfAd8 = [{ id: 1, tariffName: 'Все размеры' }];
  const tariffsByViewOfAd9 = [{ id: 1, tariffName: 'Все размеры' }];
  const tariffsByViewOfAd10 = [{ id: 1, tariffName: 'от 5 до 10 кв.м' }];

  // Обработчики для выпадающих списков
  const handleTypeChange = (event) => setSelectedType(event.target.value);
  const handleTypeChangeSelectedViewOfAd = (event) => setSelectedViewOfAd(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd = (event) => setSelectedTariffsByViewOfAd1(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd2 = (event) => setSelectedTariffsByViewOfAd2(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd3 = (event) => setSelectedTariffsByViewOfAd3(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd4 = (event) => setSelectedTariffsByViewOfAd4(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd5 = (event) => setSelectedTariffsByViewOfAd5(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd6 = (event) => setSelectedTariffsByViewOfAd6(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd7 = (event) => setSelectedTariffsByViewOfAd7(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd8 = (event) => setSelectedTariffsByViewOfAd8(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd9 = (event) => setSelectedTariffsByViewOfAd9(event.target.value);
  const handleTypeChangeSelectedTariffByViewOfAd10 = (event) => setSelectedTariffsByViewOfAd10(event.target.value);
  const handleChangeCategoryOfStreet = (e) => setCategoryOfStreet(e.target.value);

  const handleChangeRadio = (e) => setIsContainProhibitedAD(e.target.value === 'true');
  const handleChangeRadioExcludeFromReport = (e) => setExcludeFromReport(e.target.value === 'true');
  const handleChangeRadioIsRemoved = (e) => setIsAdRemoved(e.target.value === 'true');
  const handleChangeRadioIsOnListOfDismantling = (e) => setIsOnListOfDismantling(e.target.value === 'true');
  ...
  const handleSubmit = async () => {
    if (!bannerAddress || !selectedCompanyId || !selectedType || !selectedViewOfAd) {
      setError('Пожалуйста, заполните все обязательные поля.');
      return;
    }
    ...
    formData.append('bannerLongitude', markerPosition.lng);
    formData.append('bannerLatitude', markerPosition.lat);
    ...
    // Добавление тарифа в зависимости от вида рекламы
    if (selectedViewOfAd === 'Наружняя визуальная реклама за исключением видеоизображения') {
      formData.append('tariff', selectedTariffsByViewOfAd1);
    } else if (selectedViewOfAd === 'Лайтбокс (сити-формат)') {
      formData.append('tariff', selectedTariffsByViewOfAd2);
    } else if (selectedViewOfAd === 'Надкрышная световая наружная (визуальная) реклама (светодиномические панно или объемные неоновые буквы)') {
      formData.append('tariff', selectedTariffsByViewOfAd3);
    } else if (selectedViewOfAd === 'Реклама на палатках, тентах, шатрах, навесах, зонтах, флагах, вымпелах, штандартах') {
      formData.append('tariff', selectedTariffsByViewOfAd4);
    } else if (selectedViewOfAd === 'Реклама на киосках и павильонах временного типа') {
      formData.append('tariff', selectedTariffsByViewOfAd5);
    } else if (selectedViewOfAd === 'Выносная передвижная реклама') {
      formData.append('tariff', selectedTariffsByViewOfAd6);
    } else if (selectedViewOfAd === 'LED экран') {
      formData.append('tariff', selectedTariffsByViewOfAd7);
    } else if (selectedViewOfAd === 'Бегущая строка') {
      formData.append('tariff', selectedTariffsByViewOfAd8);
    } else if (selectedViewOfAd === 'Уличная мебель') {
      formData.append('tariff', selectedTariffsByViewOfAd9);
    } else if (selectedViewOfAd === 'Ситиборд') {
      formData.append('tariff', selectedTariffsByViewOfAd10);
    }
  ...
  const SearchableTable = () => {
    ...
    const selectedCompany = (id) => {
      const company = allCompaniesFromReducer.find(item => item.id === id);
      if (company) {
  ...
  return (
    <div className="flexColumn">
      {step === 1 && (
        <Row>
          <Col sm="12" xs="12">
            <Typography variant="h5" gutterBottom>Шаг 1: Выберите точку на карте</Typography>
            <GisMap onMarkerPositionChange={handleMarkerPositionChange} />
            {markerPosition && (
              <Typography>Выбрано: Широта: {markerPosition.lat}, Долгота: {markerPosition.lng}</Typography>
            )}
            {error && <Typography color="error">{error}</Typography>}
            <Button
              variant="contained"
      ...
      {step === 2 && (
        <Row>
          ...
          <Col sm="4" xs="6">
            <Row className="card">
              <Col>
                <Typography variant="h5" gutterBottom>Шаг 2: Заполните данные баннера</Typography>
                <Button
                  variant="outlined"
                  color="secondary"
                ...
                <Form>
                  ...
                  <FormControl fullWidth>
                    <InputLabel id="categoryOfStreet-label">Select CategoryOfStreet</InputLabel>
                    <Select
                      labelId="categoryOfStreet-label"
                      id="categoryOfStreet"
                      value={selectCategoryOfStreet}
                  ...
                  <br /><br />
                  {selectedViewOfAd === 'Наружняя визуальная реклама за исключением видеоизображения' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                        id="tariffs"
                        value={selectedTariffsByViewOfAd1}
                  ...
                  {selectedViewOfAd === 'Лайтбокс (сити-формат)' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Надкрышная световая наружная (визуальная) реклама (светодиномические панно или объемные неоновые буквы)' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Реклама на палатках, тентах, шатрах, навесах, зонтах, флагах, вымпелах, штандартах' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Реклама на киосках и павильонах временного типа' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Выносная передвижная реклама' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Бегущая строка' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  {selectedViewOfAd === 'Уличная мебель' && (
                    <FormControl fullWidth>
                      <InputLabel id="tariffs-label">Select Tariff</InputLabel>
                      <Select
                        labelId="tariffs-label"
                  ...
                  <fieldset>
                    ...
                    <div>
                      <input
                        type="radio"
                        id="prohibitedYes"
                        name="isContainProhibitedAD"
                        value={true}
                        checked={isContainProhibitedAD}
                        onChange={handleChangeRadio}
                      />
                      <label htmlFor="prohibitedYes">Да</label>
                    </div>
                    <div>
                      <input
                        type="radio"
                        ...
                        onChange={handleChangeRadio}
                      />
                      <label htmlFor="prohibitedNo">Нет</label>
                    </div>
                  </fieldset>
